...
int	main(void)
{
	...
	*/
	...
	if (error == 0)
		...
		printf("\e[0;31m    /!\\ KO : erreur ou echec de malloc dans %d fonction(s).\e[0m\n", error);
	return (0);
}
...
{
	char str[100] = "Ceci est un mega test, bien qu'il puisse etre encore plus long";

	ft_bzero(str + 2, 4); /*On met quatre \0 a partir du 3eme terme de str*/
	if (strcmp(str, "Ce") != 0 || *(str + 2) != '\0' || *(str + 3) != '\0' || *(str + 4) != '\0' || *(str + 5) != '\0' || strcmp(str + 2 + 4, "st un mega test, bien qu'il puisse etre encore plus long") != 0)
	{
...
int	test_ft_strchr(void)
{
	...
	if ((ft_strchr(str, 'e') != str + 1) || (ft_strchr(str, '\0') != str + strlen(str)) || (ft_strchr(str, 'w') != NULL) || (ft_strchr(str, 'e' + 256) != str + 1))
	{
...
{
	char	str[] = "Test de la fonction";
	if (ft_strrchr(str, 'e') != strrchr(str, 'e') || ft_strrchr(str, 'e' + 256) != strrchr(str, 'e' + 256))
	{
...
{
	if (ft_strncmp("Test de la fonction", "Test de la fonction!",0) != strncmp("Test de la fonction", "Test de la fonction!",0)
	|| ft_strncmp("Test de la fonction", "Test de la fonction!",24) != strncmp("Test de la fonction", "Test de la fonction!",24)
	|| ft_strncmp("Test de la fonction", "Test de la fonction!",12) != strncmp("Test de la fonction", "Test de la fonction!",12) || ft_strncmp("", "",12) != 0)
	{
...
{
	if (ft_memchr("Test de la fonction!", 't', 2) || strcmp(ft_memchr("Test de la fonction!", 't', 28),memchr("Test de la fonction!", 't', 28)) != 0)
	{
...
int	test_ft_memcmp(void)
{
	...
	/*
	//Eliminé car la fonction de base affiche un warning dans la console, ce qui pollue le visuel
	if (memcmp("", str2, n) != ft_memcmp("", str2, n))
	{
		test_msgbilan("memcmp", 0);
		return (1);
	}
...
int	test_ft_strnstr(void)
{
	...
	if (strcmp(ft_strnstr(big, little, 30),"est la botte de foin.") != 0 || ft_strnstr(big, little, 4) != NULL)
	{
...
{
	if (ft_atoi("test") != atoi("test") || ft_atoi("-12") != atoi("-12") ||  ft_atoi("2147483647") != atoi("2147483647") ||  ft_atoi("-2147483648") != atoi("-2147483648"))
	{
...
int	test_ft_strjoin(void)
{
	...
	if (strcmp(s3, "Premiere chaine de caractere!Seconde chaine de caractere!") != 0 || strcmp(s4, "Premiere chaine de caractere!") != 0)
	{
		free(s3);
		free(s4);
...
int	test_ft_split(void)
{
	...
	if (!table[0] || strcmp(table[0],"Bonjour,") != 0 || !table[1] || strcmp(table[1],"test") != 0 ||!table[2] || strcmp(table[2],"split_") != 0 || table[3])
	{
		j = 0;
		while (table[j])
			j++;
...
int	test_ft_itoa(void)
{
	...
	if (strcmp(str, "-2147483648") != 0 || strcmp(str1, "2147483647") != 0 || strcmp(str2, "0") != 0)
	{
		free(str);
		free(str1);
		free(str2);
...
static char ft_strmapi_test(unsigned int i, char c)
{
	return (c + i);
}
...
static void	ft_striteri_test(unsigned int i, char *c)
{
	*c += i;
}
