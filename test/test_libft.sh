...
then
	 echo -e "Erreur:\nle programme doit etre lance avec la commande 'bash test_libft.sh ...' (et non ./test_libft.sh ...) depuis son repertoire d'origine. Ce dernier contient egalement le repertoire test_src/ contenant les fichiers sources utilises par le testeur. -> Ne pas deplacer ou modifier les elements au sein de ces repertoires."
	 exit 1
fi
...
gcc -g test_src/test_libft_functions_auto.c -L$LIBFT_DIR -lft -o test.exe
./test.exe
rm test.exe
...
gcc -g test_src/test_libft_functions_manual.c -L$LIBFT_DIR -lft -o test.exe
./test.exe
rm test.exe
rm libft.h